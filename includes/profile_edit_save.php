<?php
	...
	/*
	 * Save the user healing data updated from their profile page
	 * */
	...
	$fullname_of_new_member = $userdata->user_firstname . ' ' . $userdata->user_lastname;
	$user_eamil = $userdata->user_email;
	
	$final_message = "$fullname_of_new_member wants to add new healing modality/modalities \n\n";
	...
	foreach($m_names as $m_name){
		...
		if($_POST[$sanitized . '_document'] == 'yes') :
			...
			$okay = 'okay';
			$link = $home ."/pie-registration/?admin=$okay&uid=$user_id&modal=$modal&authkey=$activationkey";
			...
			$link_to_approve_1 = $home . "/pie-registration/?uid=$user_id&modal=$modal&rm=$rm_1&authkey=$activationkey";
			$link_to_approve_2 = $home . "/pie-registration/?uid=$user_id&modal=$modal&rm=$rm_2&authkey=$activationkey";
	...
	wp_mail($admin_mail,'New Modality Request',$final_message,$headers);	
